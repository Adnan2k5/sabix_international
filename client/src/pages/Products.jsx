...
const Products = () => (
  <>
    ...
    <section className="section-border-top" style={{ backgroundColor: 'var(--color-background)' }}>
      <div className="container section-padding">
        <FadeIn className="mb-16 max-w-3xl">
          <SectionHeading
            eyebrow={products.intro.eyebrow}
            title={products.intro.headline}
            description={products.intro.body}
          />
        </FadeIn>
            ...
            <FadeIn key={cat.id} delay={i * 0.06}>
              <div className={`grid grid-cols-1 lg:grid-cols-2 ${i % 2 === 1 ? '' : ''}`}>
                ...
                >
                  ...
                  <h2 className="text-title mb-3" style={{ fontSize: 'clamp(1.4rem, 2.5vw, 2rem)' }}>{cat.title}</h2>
                  <p className="text-body text-[var(--color-muted)] mb-6 text-sm leading-relaxed" style={{ maxWidth: '44ch' }}>{cat.description}</p>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    ...
                    {cat.families.length > 3 && (
                      <span className="text-[11px] font-medium px-2.5 py-1 text-[var(--color-muted)]">+{cat.families.length - 3} more</span>
                    )}
                  </div>
    ...
    <section className="section-border-top" style={{ backgroundColor: 'var(--color-primary)' }}>
      <div className="container section-padding">
        <FadeIn className="mb-14">
          <SectionHeading
            eyebrow="Why SABIX"
            title="A different approach to industrial supply."
          />
        </FadeIn>
          ...
          {products.features.map((f, i) => (
            <FadeIn key={f.icon} delay={i * 0.06}>
              <div className="p-10 flex flex-col gap-4" style={{ backgroundColor: 'transparent' }}>
                ...
                <h3 className="text-subheading text-base font-semibold text-white">{f.title}</h3>
    ...
    <CTASection
      ...
      headline={products.cta.headline}
      body={products.cta.body}
      primaryLabel="Request a Quote"
      primaryTo="/contact"
      secondaryLabel="About SABIX"
      secondaryTo="/about"
    />
  </>
);
